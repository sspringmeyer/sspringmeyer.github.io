...
tab.WindowHelper.clearLocationHash = function tab_WindowHelper$clearLocationHash(loc) {
    var href = loc.href;
    href = href.substr(0, href.length - loc.hash.length);
    if (href.charAt(href.length - 1) === '#') {
        href = href.substr(0, href.length - 1);
    }
    return href;
}
tab.WindowHelper.updateURLRetryInfo = function tab_WindowHelper$updateURLRetryInfo(loc, url, addRetryInfo) {
    var searchStr = loc.search;
    var index = searchStr.indexOf(':retry=true');
    if (addRetryInfo) {
        if (index === -1) {
            if (!searchStr.length) {
                url += '?' + ':retry=true';
                ...
                url = url.replace(searchStr, searchStr + '&' + ':retry=true');
            }
        }
    }
    else {
        if (index !== -1) {
            var newSearch = '';
            if (searchStr !== '?' + ':retry=true') {
                newSearch = searchStr.replace('&' + ':retry=true', '');
            }
            url = url.replace(searchStr, newSearch);
        }
    }
    return url;
}
...
tab.WindowHelper.reloadLocation = function tab_WindowHelper$reloadLocation(addRetryInfo) {
    var url = tab.WindowHelper.clearLocationHash(window.location);
    url = tab.WindowHelper.updateURLRetryInfo(window.location, url, addRetryInfo);
    window.location.replace(url);
}
tab.WindowHelper.locationIndicatesRetry = function tab_WindowHelper$locationIndicatesRetry() {
    ...
    _handleLegacyNotifications: function tab__crossDomainMessageRouter$_handleLegacyNotifications(messageName, e, handler) {
        if (messageName === 'tableau.loadIndicatorsLoaded') {
            ...
            for (var $key2 in $dict1) {
                ...
                if (tab._Utility.hasOwnProperty(this._handlers, pair.key) && pair.value.get_iframe().contentWindow === e.source) {
...
tab._CustomViewImpl._createNew = function tab__CustomViewImpl$_createNew(workbookImpl, messagingOptions, dict, defaultId) {
    var cv = new tab._CustomViewImpl(workbookImpl, dict['name'], messagingOptions);
...
tab._CustomViewImpl._getCustomViewsAsync = function tab__CustomViewImpl$_getCustomViewsAsync(workbookImpl, messagingOptions) {
    ...
    var returnHandler = new tab._CommandReturnHandler('api.FetchCustomViewsCommand', 0, function(result) {
        var cvs = result;
        tab._CustomViewImpl._processCustomViews(workbookImpl, messagingOptions, cvs);
...
tab._CustomViewImpl.prototype = {
    ...
    _removeAsync: function tab__CustomViewImpl$_removeAsync() {
        ...
        var returnHandler = tab._CustomViewImpl._createCustomViewCommandReturnHandler('api.RemoveCustomViewCommand', deferred, ss.Delegate.create(this, function(result) {
            this._isStale = true;
            var cvs = result;
            tab._CustomViewImpl._processCustomViews(this._workbookImpl, this._messagingOptions, cvs);
            deferred.resolve(this.get__customView());
        }));
        this._messagingOptions.sendCommand(param, returnHandler);
        return deferred.get_promise();
    ...
    _showAsync: function tab__CustomViewImpl$_showAsync() {
        ...
        return tab._CustomViewImpl._showCustomViewAsync(this._workbookImpl, this._messagingOptions, this._serverCustomizedView);
    },
...
tab._StoryPointImpl.createContainedSheet = function tab__StoryPointImpl$createContainedSheet(containedSheetInfo, workbookImpl, messagingOptions, findSheetFunc) {
    var containedSheetType = tab._WorkbookImpl._apiCrossDomainSheetTypeToSheetType(containedSheetInfo.sheetType);
    var index = -1;
    var size = tab.SheetSizeFactory.createAutomatic();
    var isActive = false;
...
tab._Utility.isSafari = function tab__Utility$isSafari() {
    ...
    return ua.indexOf('Safari') >= 0 || (isWebKit && !isChrome);
}
...
tab.VizImpl.prototype = {
    ...
    hideLoadIndicators: function tab_VizImpl$hideLoadIndicators() {
    ...
    handleEventNotification: function tab_VizImpl$handleEventNotification(eventName, eventParameters) {
        ...
        if (eventName === 'api.VizInteractiveEvent') {
            if (ss.isValue(this._workbookImpl) && this._workbookImpl.get_name() === notif.get__workbookName()) {
                this._onWorkbookInteractive();
            }
            ...
            if (this.__marksSelection != null) {
                if (this._workbookImpl.get_name() === notif.get__workbookName()) {
                    var worksheetImpl = null;
                    var activeSheetImpl = this._workbookImpl.get_activeSheetImpl();
                    if (activeSheetImpl.get_name() === notif.get__worksheetName()) {
                        worksheetImpl = activeSheetImpl;
                    }
                    else if (activeSheetImpl.get_isDashboard()) {
                        var db = activeSheetImpl;
                        worksheetImpl = db.get_worksheets()._get(notif.get__worksheetName())._impl;
                    }
            ...
            if (ss.isValue(this._workbookImpl)) {
                tab._CustomViewImpl._processCustomViews(this._workbookImpl, this._messagingOptions, dict);
            }
            this._raiseCustomViewsListLoad();
            if (this.__customViewLoad != null && !dict['customViewLoaded']) {
            ...
            if (ss.isValue(this._workbookImpl)) {
                tab._CustomViewImpl._processCustomViewUpdate(this._workbookImpl, this._messagingOptions, dict, true);
            }
            ...
            if (storyImpl.get_sheetType() === 'story') {
                storyImpl.update(JSON.parse(notif.get__data()));
            }
        }
    },
    ...
    _setIframeSrc: function tab_VizImpl$_setIframeSrc(loadOrder) {
    ...
    _onWorkbookInteractive: function tab_VizImpl$_onWorkbookInteractive() {
...
tab._WorkbookImpl._apiCrossDomainSheetTypeToSheetType = function tab__WorkbookImpl$_apiCrossDomainSheetTypeToSheetType(sheetType) {
    switch (sheetType) {
        case 'dashboard':
            return 'dashboard';
        case 'story':
        ...
        case 'worksheet':
            return 'worksheet';
        default:
            throw tab._TableauException.createInternalError("Unknown sheetType '" + sheetType + "'");
    }
}
...
tab._WorkbookImpl._createSheetSize = function tab__WorkbookImpl$_createSheetSize(sheetInfo) {
    ...
    var minHeight = sheetInfo.minHeight;
    var minWidth = sheetInfo.minWidth;
    var maxHeight = sheetInfo.maxHeight;
    var maxWidth = sheetInfo.maxWidth;
    ...
    _createSheetsInfo: function tab__WorkbookImpl$_createSheetsInfo(clientInfo) {
        ...
        for (var index = 0; index < baseSheets.length; index++) {
            ...
            if (ss.isNullOrUndefined(sheetInfo)) {
                ...
                var sheetType = tab._WorkbookImpl._apiCrossDomainSheetTypeToSheetType(baseSheet.sheetType);
...
tab._WorksheetImpl._filterCommandError = function tab__WorksheetImpl$_filterCommandError(errorData) {
    if (ss.isValue(errorData['invalidFieldCaption'])) {
        return tab._TableauException.create('invalidFilterFieldName', errorData['invalidFieldCaption']);
    }
    else if (ss.isValue(errorData['invalidValues'])) {
        return tab._TableauException.create('invalidFilterFieldValue', errorData['invalidValues']);
    }
    else if (ss.isValue(errorData['invalidAggFieldName'])) {
        var fieldName = errorData['fieldCaption'];
        return tab._TableauException._createInvalidAggregationFieldName(fieldName);
    }
    return null;
}
...
tab._WorksheetImpl._createSelectionCommandError$1 = function tab__WorksheetImpl$_createSelectionCommandError$1(errorData) {
    if (ss.isValue(errorData['invalidFields'])) {
        return tab._TableauException.create('invalidSelectionFieldName', errorData['invalidFields']);
    }
    else if (ss.isValue(errorData['invalidValues'])) {
        return tab._TableauException.create('invalidSelectionValue', errorData['invalidValues']);
    }
    else if (ss.isValue(errorData['invalidDates'])) {
        return tab._TableauException.create('invalidSelectionDate', errorData['invalidDates']);
    }
    return null;
}
tab._WorksheetImpl.prototype = {
    ...
    _getDataSourcesAsync: function tab__WorksheetImpl$_getDataSourcesAsync() {
        ...
        var deferred = new tab._Deferred();
        var commandParameters = {};
        var returnHandler = new tab._CommandReturnHandler('api.GetDataSourcesCommand', 0, ss.Delegate.create(this, function(result) {
            var dataSourcesDict = result;
            var dataSources = tab._dataSourceImpl._processDataSources(dataSourcesDict, this.get_name());
            this.set__dataSources(dataSources);
    ...
    _getOneDataSourceAsync: function tab__WorksheetImpl$_getOneDataSourceAsync(dataSourceName) {
        this._verifyActiveSheetOrEmbeddedInActiveDashboard$1();
        var deferred = new tab._Deferred();
        var param = {};
        var returnHandler = new tab._CommandReturnHandler('api.GetDataSourcesCommand', 0, ss.Delegate.create(this, function(result) {
            ...
            deferred.reject(tab._TableauException.create('serverError', 'datasource not found'));
        }), function(remoteError, message) {
            deferred.reject(tab._TableauException.createServerError(message));
        });
        this.sendCommand(param, returnHandler);
...
tab.CustomViewEvent.prototype = {
    ...
    getCustomViewAsync: function tab_CustomViewEvent$getCustomViewAsync() {
        var deferred = new tab._Deferred();
        deferred.resolve(this._context$1.get__customViewImpl().get__customView());
        return deferred.get_promise();
    }
}
    ...
    getFieldAsync: function tableauSoftware_Filter$getFieldAsync() {
        ...
        if (this._field == null) {
            ...
            var fulfilled = ss.Delegate.create(this, function(value) {
                this._field = new tableauSoftware.Field(value, this._caption, this._fieldRoleType, this._fieldAggrType);
                deferred.resolve(this._field);
                return null;
            });
            this._worksheetImpl._getOneDataSourceAsync(this._datasourceName).then(fulfilled, rejected);
        }
        else {
            window.setTimeout(ss.Delegate.create(this, function() {
                deferred.resolve(this._field);
